...
df = df_randomized[:1000]

# Get embeddings
embeddings = get_improved_embeddings(df['content'].tolist())
embeddings_np = embeddings.numpy()

# Try multiple clustering approaches
clustering_results = try_multiple_clustering(embeddings_np)

# Pick HDBSCAN results (or whichever method works best)
df['cluster'] = clustering_results['hdbscan']

# Print cluster quality metrics
for method, labels in clustering_results.items():
    if method != 'umap_coords':
        quality = analyze_cluster_quality(embeddings_np, labels)
        if quality:
            print(f"\n{method.upper()} Clustering Quality:")
            print(f"Number of clusters: {quality['n_clusters']}")
            print(f"Silhouette score: {quality['silhouette']:.3f}")
            print(f"Noise points: {quality['noise_points']}")

# Print representative dreams from each cluster
unique_clusters = sorted(list(set(clustering_results['hdbscan'])))
for cluster_num in unique_clusters:
    if cluster_num != -1:  # Skip noise points
        print(f"\nCluster {cluster_num}:")
        cluster_dreams = df[df['cluster'] == cluster_num]['content']
        
        # Get cluster members
        cluster_indices = np.where(df['cluster'] == cluster_num)[0]
        cluster_embeddings = embeddings_np[cluster_indices]
        
        # Find dreams closest to cluster center
